...
if repos['values'].find { |repo| repo['name'] == REPO_NAME }
  ...
  run %{curl -X POST https://hooks.slack.com/services/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 -d '{"text": "@botanicus:\n1. Please create <https://registry.hub.docker.com/builds/bitbucket/botanicus/#{REPO_NAME}/|#{REPO_NAME} automated build>. Dockerhub does not have API for this. Also add https://registry.hub.docker.com/hooks/bitbucket to <https://bitbucket.org/botanicus/#{REPO_NAME}/admin/hooks|#{REPO_NAME} hooks> (type POST).\n2. Add <https://registry.hub.docker.com/u/paypertask/#{REPO_NAME}/settings/webhooks/add|Dockerhub webhooks> \n3. Then deploy it <https://dashboard.tutum.co/container/launch/#tab-image-privates|from private> or <https://dashboard.tutum.co/container/launch/#tab-image-community|from public build>. Do not forget to\n  - Expose and map ports and make them published if necessary.\n  - Turn autorestart on.\n  - Maybe some links and volumes?", "username": "Deployment notifications", "channel": "#general"}'}
