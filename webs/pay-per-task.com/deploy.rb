...
if repos['values'].find { |repo| repo['name'] == REPO_NAME }
  ...
  system "curl -X POST --user #{BITBUCKET_CREDENTIALS} #{BITBUCKET_API}/repositories/botanicus/#{REPO_NAME}"
  ...
  echo ">>> CREATE automated build. So far this isn't supported in the API."
